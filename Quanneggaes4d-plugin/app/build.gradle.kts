...
android {
    ...
    namespace = "com.quanneggaes4d.plugin"
    compileSdk = 36

    ndkVersion = "27.3.13750724"

    defaultConfig {
        applicationId = "com.quanneggaes4d.plugin"
        minSdk = 26
        targetSdk = 36
        versionCode = 2000
        versionName = "2.0.0"
    ...
    buildTypes {
        ...
        configureEach {
            resValue("string","app_name","QUANNEGGAES4D")

            manifestPlaceholders["des"] = "QUANNEGGAES4D (OpenGL 4.0, 1.17+)"
            manifestPlaceholders["renderer"] = "QUANNEGGAES4D:libquanneggaes4d.so:libquanneggaes4d.so"

            manifestPlaceholders["minMCVer"] = "1.17"
            manifestPlaceholders["maxMCVer"] = "" //为空则不限制 No restriction if empty

            // QNG_COUNT_LAUNCH：只有经启动器起来的这一次才算「一次启动」。
            // 基准测试、各种工具、以及本插件自己 dlopen 一次去读 GL 信息，都不该计数。
            manifestPlaceholders["boatEnv"] = mutableMapOf<String,String>().apply {
                put("LIBGL_ES", "3")
                put("QNG_COUNT_LAUNCH", "1")
            }.run {
                var env = ""
                forEach { (key, value) ->
                    env += "$key=$value:"
            ...
            manifestPlaceholders["pojavEnv"] = mutableMapOf<String,String>().apply {
				...
				put("POJAVEXEC_EGL", "libquanneggaes4d.so")
				put("LIBGL_EGL", "libquanneggaes4d.so")
                put("QNG_COUNT_LAUNCH", "1")
            }.run {
                var env = ""
                forEach { (key, value) ->
...
dependencies {
    ...
    implementation(project(":QUANNEGGAES4D"))
