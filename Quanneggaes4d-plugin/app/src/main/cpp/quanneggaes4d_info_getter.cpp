#include "quanneggaes4d_info_getter.h"
...
static bool load_mobile_symbols() {
    ...
    mg_handle = dlopen("libquanneggaes4d.so", RTLD_NOW | RTLD_LOCAL);
    if (!mg_handle) {
        fprintf(stderr, "Error: dlopen libquanneggaes4d.so failed: %s", dlerror());
        return false;
    }
...
static std::string create_context_and_query() {
    if (!load_mobile_symbols()) return "Error: failed to load libquanneggaes4d symbols";
    ...
    if (numExt > 0 && p_glGetStringi) {
        ...
        for (GLint i = 0; i < numExt; ++i) {
            ...
            if (ext) {
                ...
                if (strcmp(extStr, "GL_QNG_quanneggaes4d") == 0) g_MGQueryCapability.HasQUANNEGGAES4DExt = true;
                if (strcmp(extStr, "GL_QNG_backend_string_getter_access") == 0) g_MGQueryCapability.BackendStringGetterAccess = true;
                if (strcmp(extStr, "GL_QNG_settings_string_dump") == 0) g_MGQueryCapability.SettingsStringDump = true;
        ...
        if (exts) {
            ...
            if (strstr(extStr, " GL_QNG_quanneggaes4d ")) g_MGQueryCapability.HasQUANNEGGAES4DExt = true;
            if (strstr(extStr, " GL_QNG_backend_string_getter_access ")) g_MGQueryCapability.BackendStringGetterAccess = true;
            if (strstr(extStr, " GL_QNG_settings_string_dump ")) g_MGQueryCapability.SettingsStringDump = true;
        }
    }

    out << "Is QUANNEGGAES4D (>=1.3.3): " << (g_MGQueryCapability.HasQUANNEGGAES4DExt ? "Yes\n" : "No\n");
    ...
    typedef int (*PFN_qng_angle_in_use)();
    auto p_angle_in_use = (PFN_qng_angle_in_use) dlsym(mg_handle, "qng_angle_in_use");
    if (p_angle_in_use) {
    ...
    if (g_MGQueryCapability.BackendStringGetterAccess) {
        const GLubyte* backendRenderer = p_glGetString(GL_RENDERER + GL_QNG_BACKEND_GETTER);
        const GLubyte* backendVersion = p_glGetString(GL_VERSION + GL_QNG_BACKEND_GETTER);
        const GLubyte* backendVendor = p_glGetString(GL_VENDOR + GL_QNG_BACKEND_GETTER);
        const GLubyte* backendShading = p_glGetString(GL_SHADING_LANGUAGE_VERSION + GL_QNG_BACKEND_GETTER);
        ...
        p_glGetIntegerv(GL_NUM_EXTENSIONS + GL_QNG_BACKEND_GETTER, &backendNumExt);
        if (backendNumExt > 0 && p_glGetStringi) {
            out << "Extensions (" << backendNumExt << "):\n";
            for (GLint i = 0; i < backendNumExt; ++i) {
                const GLubyte* ext = p_glGetStringi(GL_EXTENSIONS + GL_QNG_BACKEND_GETTER, (GLuint)i);
                out << "  " << reinterpret_cast<const char*>(ext) << "\n";
            }
        } else {
            const GLubyte* exts = p_glGetString(GL_EXTENSIONS + GL_QNG_BACKEND_GETTER);
            out << "Extensions: " << (exts ? reinterpret_cast<const char*>(exts) : "NULL") << "\n";
        }
    } else {
        out << "Not Compatible. GL_QNG_backend_string_getter_access is not supported in current context.\n";
    }

    out << "\nSettings\n====\n";
    if (g_MGQueryCapability.SettingsStringDump) {
        const GLubyte* settingsStr = p_glGetString(GL_QNG_SETTINGS);
        out << reinterpret_cast<const char*>(settingsStr) << "\n";
    } else {
        out << "Not Compatible. GL_QNG_settings_string_dump is not supported in current context.\n";
    }
...
Java_com_quanneggaes4d_plugin_QNGInfoGetter_getQuanneggaes4dGLInfo(JNIEnv *env,
                                                                          jobject thiz) {
    std::string res = create_context_and_query();
    printf("QUANNEGGAES4D GL Info: \n%s", res.c_str());
    return env->NewStringUTF(res.c_str());
}

// Runs the MultiDraw micro benchmark that lives inside libquanneggaes4d
...
typedef int (*PFN_qng_multidraw_bench_progress)();
static std::mutex g_bench_progress_mutex;
static PFN_qng_multidraw_bench_progress p_bench_progress = nullptr;

static void set_bench_progress_fn(PFN_qng_multidraw_bench_progress fn) {
...
static std::string create_context_and_bench(int start_sections, int max_sections) {
    if (!load_mobile_symbols()) return R"({"error":"failed to load libquanneggaes4d symbols"})";

    typedef const char* (*PFN_qng_multidraw_bench_run)(int, int);
    auto p_bench = (PFN_qng_multidraw_bench_run) dlsym(mg_handle, "qng_multidraw_bench_run");
    if (!p_bench) {
        return R"({"error":"qng_multidraw_bench_run is missing; the renderer is too old"})";
    }
    ...
    auto p_progress = (PFN_qng_multidraw_bench_progress) dlsym(mg_handle, "qng_multidraw_bench_progress");
    ...
    // Copy before teardown: the string lives inside libquanneggaes4d.
...
extern "C"
JNIEXPORT jstring JNICALL
Java_com_quanneggaes4d_plugin_QNGBench_runMultidrawBench(JNIEnv *env, jobject thiz,
                                                          jint startSections, jint maxSections) {
    std::string res = create_context_and_bench(startSections, maxSections);
    printf("QUANNEGGAES4D MultiDraw bench: \n%s", res.c_str());
    __android_log_print(ANDROID_LOG_INFO, "QNGBench", "%s", res.c_str());
    return env->NewStringUTF(res.c_str());
}
...
Java_com_quanneggaes4d_plugin_QNGBench_benchProgress(JNIEnv *env, jobject thiz) {
    (void)env;
    (void)thiz;
    std::lock_guard<std::mutex> lock(g_bench_progress_mutex);
...
extern "C"
JNIEXPORT jint JNICALL
Java_com_quanneggaes4d_plugin_QNGInfoGetter_setenv(JNIEnv *env, jobject thiz, jstring key,
