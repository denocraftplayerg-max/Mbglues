 ...
 *
 * 渲染器只在 libquanneggaes4d 第一次进入进程时读环境和配置：QNG_ANGLE_DIR、config.json
 ...
 */
class QngQueryService : Service() {

    private val binder = object : IQngQuery.Stub() {
        ...
        override fun benchProgress(): Int = QNGBench.rawProgress()

        override fun glInfo(mgDirectory: String, angleDirectory: String): String =
            QNGInfoGetter.info(File(mgDirectory), angleDirectory.takeUnless { it.isEmpty() })
    }
