 ...
 * 对 `MG/` 目录的访问抽象。
 *
 * 两种实现：[DirectQngStorage]（「所有文件访问」或旧版存储权限，直接走文件系统）和
 * [SafQngStorage]（SAF 目录授权，走 DocumentsContract）。native 端永远读 `/sdcard/QNG` 的真实路径，
 * 这个抽象只对 App 自己的读写生效——两种授权方式写出来的是同一个目录、同一个 config.json。
 *
 * 所有函数都可能做 IO，调用方负责放到后台线程。
 */
interface QngStorage {

    /** 给用户看的路径，两种模式下都指 `/sdcard/QNG`。 */
     ...
     * 写入配置全文。
     *
     * [DirectQngStorage] 是原子写（临时文件 + rename）；SAF 没有 rename 可用，
     * [SafQngStorage] 只能截断式覆写，是尽力而为。
     */
    @Throws(IOException::class)
    ...
    /**
     * 删除 QUANNEGGAES4D 和本插件在 MG 目录下自己创建的全部文件（危险区域的「移除 QUANNEGGAES4D」）。
     *
...
class DirectQngStorage(private val root: File) : QngStorage {
...
class SafQngStorage(
    ...
    companion object {
        const val MG_DIRECTORY_NAME = "QNG"
...
/**
 * QUANNEGGAES4D（native 库）和本插件会在 MG 目录下主动创建的全部文件名。
 *
 ...
 * 先写临时文件再 rename。
 *
 * 同目录下的 rename 是原子的，所以读的一方（游戏里的 QUANNEGGAES4D）要么看到旧内容，
