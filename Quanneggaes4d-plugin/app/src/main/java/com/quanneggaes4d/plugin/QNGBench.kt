...
object QNGBench {
     ...
     *
     * 与 [QNGInfoGetter.info] 同一条通道：dlopen libquanneggaes4d，经渲染器自己的 EGL 层
     * 建上下文，调用 `qng_multidraw_bench_run`，拿回一段 JSON。阻塞且耗时（默认预算 8 秒，
     * 见 native 侧的 MG_BENCH_BUDGET_MS），必须放在后台线程。
     *
     * 只应从 [QngQueryService] 的进程里调用：环境变量和配置都在库的静态构造里读，
    ...
    fun run(
        ...
        // 跑分不是一次「启动」，不设 QNG_COUNT_LAUNCH。
        QNGInfoGetter.setenv("QNG_PLUGIN_STATUS", "1", 1)
        QNGInfoGetter.setenv("QNG_DIR_PATH", mgDirectory.path, 1)
        // 空串等于没设：渲染器那边只认非空值，游戏进程里本来也不会有这个变量。
        QNGInfoGetter.setenv("QNG_ANGLE_DIR", angleDirectory.orEmpty(), 1)
