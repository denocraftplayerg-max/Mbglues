 ...
 *
 * [wire] 是磁盘上的整数，必须与 QUANNEGGAES4D native 端 `config/settings.h` 中对应的枚举一致，
...
/**
 * QUANNEGGAES4D 的配置，一个不可变的值。
 *
 * 它不知道文件、不知道 UI、也没有任何副作用：改配置就是 [copy]，落盘由 [QNGConfigStore] 负责。
 * 这里的默认值是全 App 唯一的一份——[QNGConfigCodec] 解析时的回落值也取自这里。
 */
data class QNGConfig(
