...
/** [QNGConfigStore.load] 的结果。加载失败不等于「用默认值覆盖」，由调用方决定怎么办。 */
sealed interface ConfigLoadResult {

    /** 配置文件不存在。此时 [QNGConfigStore.config] 会变成默认值，但磁盘上什么都没写。 */
    data object Missing : ConfigLoadResult

    data class Loaded(val config: QNGConfig) : ConfigLoadResult
 ...
 *  - 直接文件访问下写入是原子的（临时文件 + fsync + rename，见 [QngStorage.writeConfig]）。
 *    native 端会在游戏启动时读这个文件，截断式写入可能让它读到半截 JSON。
 *  - 写入失败通过 [events] 上报，不再被 `runCatching {}` 静默吞掉。
 */
class QNGConfigStore(
    storage: QngStorage?,
     ...
     */
    @Volatile
    private var storage: QngStorage? = storage

    private val mutableConfig = MutableStateFlow<QNGConfig?>(null)

    /** `null` = 尚未加载。 */
    val config: StateFlow<QNGConfig?> = mutableConfig.asStateFlow()
    ...
    fun attachStorage(storage: QngStorage) {
    ...
    private fun requireStorage(): QngStorage =
     ...
     * 「界面回灌触发的回调」天然是幂等的。
     */
    fun update(transform: (QNGConfig) -> QNGConfig) {
    ...
    suspend fun resetToDefaults() {
        foreignKeys = null
        mutableConfig.value = QNGConfig.Default
        dirty = true
        persist()
    }
    ...
    /** 把当前配置导出到应用私有目录（供 QNGInfoGetter 读取），不影响用户的配置文件。 */
    suspend fun exportTo(file: File): Result<File> = withContext(io) {
        runCatching {
            file.parentFile?.mkdirs()
            file.writeText(serialize(mutableConfig.value ?: QNGConfig.Default))
            file
        }
    }
    ...
    private fun readConfigFile(): ConfigLoadResult {
        ...
        foreignKeys = QNGConfigCodec.foreignKeysOf(root)
    ...
    private fun serialize(config: QNGConfig): String =
        gson.toJson(QNGConfigCodec.encode(config, foreignKeys))
