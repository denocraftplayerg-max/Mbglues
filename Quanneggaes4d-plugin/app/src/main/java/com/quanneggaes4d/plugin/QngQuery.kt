...
/**
 * 把一次渲染器查询放进一次性的 [QngQueryService] 进程里执行。
 *
 * 为什么不能就地跑：见 [QngQueryService] 顶部——库进了进程就出不去，环境和配置只被
 ...
 */
class QngQuery(private val context: Context) {
     ...
     */
    suspend fun <T> use(block: suspend (IQngQuery) -> T): T = mutex.withLock {
        val ready = CompletableDeferred<IQngQuery>()
        val connection = object : ServiceConnection {
            override fun onServiceConnected(name: ComponentName?, service: IBinder?) {
                ready.complete(IQngQuery.Stub.asInterface(service))
            }
        ...
        val requested = context.bindService(
            Intent(context, QngQueryService::class.java),
            connection,
